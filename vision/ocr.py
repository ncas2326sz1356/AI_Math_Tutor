...
def _call_gemini_api(prompt: str, api_key: str, model: str = "gemini-2.5-flash") -> str:
    """Call Google Generative Language REST endpoint (Gemini 2.5 Flash) with an API key.

    Note: `api_key` should be provided through environment variables. This helper
    uses the `generativelanguage` v1beta endpoint and is written to avoid logging the key.
    """
    try:
...
def llm_convert_to_latex(ocr_text: str, image=None, model="gpt-4o-mini") -> str:
    ...
    if gemini_key:
        try:
            ...
            out = _call_gemini_api(prompt, gemini_key, model="gemini-2.5-flash")
            if out:
                return out.strip()
        except Exception:
